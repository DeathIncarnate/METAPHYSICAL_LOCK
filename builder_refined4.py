...
from oracle_four_step import four_step_realizable_mod2_24
...
def build_edges_refined4(m:int, L:int, w:int, alpha1:int, alpha2:int,
                         forbid_k1_after_k1:bool=True,
                         prune_committed_by_oracle:bool=True):
    states, idx = build_states4(m,L,w)
    MOD = 1<<(m+2)
    ...
    for (r,t,h,phi), u in idx.items():
        ...
        if phi==0:
            if kval==1:
                ...
                add(u,v,1,t)
                # committed 4-step branch only from high bins
                if t not in low_bins:
                    keep_committed = True
                    if prune_committed_by_oracle:
                        key = (r,t)
                        if key not in oracle_cache:
                            keep = four_step_realizable_mod2_24(r, t, m, L, alpha1, alpha2)
            ...
            # must take k2 to phase t+2*alpha1+alpha2, then finish
            if kval==2 and tprime == (t + 2*alpha1 + alpha2) % L:
                v4n = idx[(y,tprime,hprime,4)]
                add(u,v4n,2,t)
        else:
            # phi==4: drop back to free with any valid kval
            if kval==1:
                v = idx[(y,tprime,hprime,0)]
                add(u,v,1,t)
            elif kval==2:
